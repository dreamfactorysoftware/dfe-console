...
class UsageService extends BaseService implements MetricsProvider
{
    ...
    {
        if (null === ($_key = config('metrics.install-key'))) {
            try {
                /** @type ServiceUser $_user */
                $_user = ServiceUser::firstOrFail();

                $_key = $this->installKey = sha1(config('provisioning.default-domain') . $_user->getHashedEmail());
            ...
            file_put_contents(config_path('metrics.php'),
                '<?php' . PHP_EOL . '// This file is automatically generated.' . PHP_EOL . "return ['install-key' => '" . $_key . "',];");
        }
