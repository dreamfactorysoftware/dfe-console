...
trait AuthenticatesAndRegistersUsers
{
    ...
    public function postLogin(Request $request)
    {
        ...
        return redirect('/auth/login')->withInput($request->only('email'))->withErrors(array('email' => 'These credentials do not match our records.'));
    }
    ...
    protected function createQueuedHandlerCallable($class, $method)
    {
        return function () use($class, $method) {
            if (method_exists($class, 'queue')) {
                ...
                $this->resolveQueue()->push('Illuminate\\Events\\CallQueuedHandler@call', array('class' => $class, 'method' => $method, 'data' => serialize(func_get_args())));
            }
        };
    }
