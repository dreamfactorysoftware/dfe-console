...
use DreamFactory\Library\Fabric\Database\Models\Auth;
...
class UserController extends ResourceController //FactoryController //
{
    ...
    public function store()
    {
        ...
        if($is_system_admin == 'true'){
            if(Deploy\ServiceUser::where('email_addr_text', '=', Input::get('email_addr_text'))->exists()){
                return 'EXISTS';
            }
            $create_user = new Deploy\ServiceUser;
        }
        else{
            if(Auth\User::where('email_addr_text', '=', Input::get('email_addr_text'))->exists()){
                return 'EXISTS';
            }
            $create_user = new Auth\User;
        }
    ...
    public function update($id)
    {
        ...
        if($is_system_admin == 'true'){
            $user = Deploy\ServiceUser::where('email_addr_text', '=', Input::get('email_addr_text'))->first();
            if($user != null){
                if($user->id != $id)
                    return 'FAIL';
            }
            ...
            $user = Auth\User::where('email_addr_text', '=', Input::get('email_addr_text'))->first();
            if($user != null){
                if($user->id != $id)
                    return 'FAIL';
            }
    ...
    {
        $a_users = new Deploy\ServiceUser;
        $o_users = new Auth\User;;
    ...
    {
        $users_owners = new Deploy\ServiceUser;
        $users_admins = new Auth\User;
        ...
        foreach($result as $value){
            ...
            $manage = '<div><input type="hidden" id="user_id" value="'.$value->id.'"><input type="hidden" id="user_admin" value="'.$value->admin.'">';
            $role = '';

            if($value->admin){
                $manage .= '<input type="checkbox" value="'.$value->id.'_admin" id="user_checkbox_'.$value->id.'_admin">&nbsp;&nbsp;
                            <button type="button" class="btn btn-default btn-xs fa fa-fw fa-trash" id="user_button_'.$value->id.'_admin" onclick="confirmRemoveUser('.$value->id.', true)" value="delete" style="width: 25px"></button>&nbsp;&nbsp;
                            <button type="button" class="btn btn-default btn-xs" id="user_button_cancel_'.$value->id.'_admin" onclick="cancelRemoveUser('.$value->id.', true)" value="delete" style="display: none">Cancel</button>';
                $role = '<span class="label label-primary" id="user_type">System Administrator</span>';
            }
            else{
                $manage .= '<input type="checkbox" value="'.$value->id.'" id="user_checkbox_'.$value->id.'">&nbsp;&nbsp;
                            <button type="button" class="btn btn-default btn-xs fa fa-fw fa-trash" id="user_button_'.$value->id.'" onclick="confirmRemoveUser('.$value->id.', false)" value="delete" style="width: 25px"></button>&nbsp;&nbsp;
                            <button type="button" class="btn btn-default btn-xs" id="user_button_cancel_'.$value->id.'" onclick="cancelRemoveUser('.$value->id.', false)" value="delete" style="display: none">Cancel</button>';
